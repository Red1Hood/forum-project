...
def ask_ai(request):
    if request.method == "POST":
        try:
            ...
            # Используем эту модель, т.к. она работает с текущим ключом
            model = genai.GenerativeModel('gemini-3.5-flash')
            
            # Немного промпта, чтобы бот не писал поэмы
            prompt = f"Ответь на вопрос кратко и дружелюбно: {user_message}"
            
            response = model.generate_content(prompt)
            
            # Возвращаем ответ в формате JSON, чтобы JS смог его прочитать
            return JsonResponse({"reply": response.text})

        except Exception as e:
            # Если что-то упало (мертвые сервера, плохой ключ), возвращаем текст ошибки в чат
